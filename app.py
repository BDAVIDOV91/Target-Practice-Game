...
def introduction():
    ...
    BTN_SUBMIT = tk.Button(master=FRM_USER_INFO, text='Submit', command=gameLevels, font=('Arial', 12))
    BTN_SUBMIT.pack()
...
def gameLevels():
    ...
    gameSettings['username'] = ENT_USERNAME.get()
    ...
    BTN_EASY = tk.Button(
        ...
        height=3, width=15,
        font=('Arial', 12),
        command=lambda : gameLayout(gameSettings['username'], gameSettings['easy'])
    )

    BTN_INTERMADIATE = tk.Button(
        master=FRM_LEVELS,
        text='Intermadiate',
        height=3, width=15,
        font=('Arial', 12),
        command=lambda : gameLayout(gameSettings['username'], gameSettings['intermadiate'])
    )
    ...
    BTN_HARD = tk.Button(
        ...
        height=3, width=15,
        font=('Arial', 12),
        command=lambda : gameLayout(gameSettings['username'], gameSettings['hard'])
    )

    BTN_EASY.pack(side=tk.TOP, fill=tk.BOTH)
    BTN_INTERMADIATE.pack(side=tk.TOP, fill=tk.BOTH)
    BTN_HARD.pack(side=tk.TOP, fill=tk.BOTH)


def gameLayout(username: str, difficulty: List):
    global FRM_INFO
    global LBL_USERNAME
    global LBL_POINTS
    global LBL_TIME
    ...
    BTN_TARGET = tk.Button(
        ...
        width=difficulty['x'],
        borderwidth=3,
        command=lambda : btnPosition(difficulty),
        bg='black'
    )
...
def btnPosition(gameLevel):
    x_axis = random.randint(0, gameLevel['x_axis_max'])
    y_axis = random.randint(0, gameLevel['y_axis_max'])
    LBL_POINTS['text'] += 1
